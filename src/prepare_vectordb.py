...
class PrepareVectorDB:
    ...
    def run(self):
        ...
        if not os.path.exists(self.vectordb_dir):
            ...
            query="Amarok the lone wolf give me some details about that"
...
if __name__ == "__main__":
    ...
    with open("config/tools_config.yml") as cfg:
        app_config = yaml.load(cfg, Loader=yaml.FullLoader)

    chunk_size = app_config["Document_rag"]["chunk_size"]
    chunk_overlap = app_config["Document_rag"]["chunk_overlap"]
    embedding_model = app_config["Document_rag"]["embedding_model"]
    vectordb_dir = app_config["Document_rag"]["vectordb"]
    collection_name = app_config["Document_rag"]["collection_name"]
    doc_dir = app_config["Document_rag"]["unstructured_docs"]
